...
export const loadEnvs = (env = process.env) => {
	...
	return {
		apiURL: env.API_URL ?? `http://localhost:${config.PORT}`,
		apiPort: env.API_PORT || 8000,
		tunnelPort: env.API_PORT || 8000,
		...
		rateLimitWindow:
			(env.RATELIMIT_WINDOW && parseInt(env.RATELIMIT_WINDOW)) || 60,
		rateLimitMax: (env.RATELIMIT_MAX && parseInt(env.RATELIMIT_MAX)) || 20,

		tunnelRateLimitWindow:
			(env.TUNNEL_RATELIMIT_WINDOW && parseInt(env.TUNNEL_RATELIMIT_WINDOW)) || 60,
		tunnelRateLimitMax:
			(env.TUNNEL_RATELIMIT_MAX && parseInt(env.TUNNEL_RATELIMIT_MAX)) || 40,

		sessionRateLimitWindow:
			(env.SESSION_RATELIMIT_WINDOW && parseInt(env.SESSION_RATELIMIT_WINDOW)) ||
			60,
		sessionRateLimit:
		...
		sessionEnabled:
			env.TURNSTILE_SITEKEY && env.TURNSTILE_SECRET && env.JWT_SECRET,

		apiKeyURL: env.API_KEY_URL && new URL(env.API_KEY_URL),
		authRequired: env.API_AUTH_REQUIRED === "1",
		redisURL: env.API_REDIS_URL,
		instanceCount:
			(env.API_INSTANCE_COUNT && parseInt(env.API_INSTANCE_COUNT)) || 1,
...
export const validateEnvs = async env => {
	if (env.sessionEnabled && env.jwtSecret.length < 16) {
		throw new Error(
			"JWT_SECRET env is too short (must be at least 16 characters long)"
		);
	}
	...
	if (env.instanceCount > 1 && !env.redisURL) {
		...
		console.error(
			"API_INSTANCE_COUNT is not supported in your environment. to use this env, your node.js"
		);
		console.error(
			"version must be >= 23.1.0, and you must be running a recent enough version of linux"
		);
		console.error(
			"(or other OS that supports it). for more info, see `reusePort` option on"
		);
	...
	if (
		...
		console.error(
			"CUSTOM_INNERTUBE_CLIENT is invalid. Provided client is not supported."
		);
		console.error(
			`Supported clients are: ${Constants.SUPPORTED_CLIENTS.join(", ")}\n`
		);
		throw new Error("Invalid CUSTOM_INNERTUBE_CLIENT");
	}

	if (
		env.forceLocalProcessing &&
		!forceLocalProcessingOptions.includes(env.forceLocalProcessing)
	) {
		console.error("FORCE_LOCAL_PROCESSING is invalid.");
		console.error(
			`Supported options are are: ${forceLocalProcessingOptions.join(", ")}\n`
		);
		throw new Error("Invalid FORCE_LOCAL_PROCESSING");
	}

	if (
		env.enableDeprecatedYoutubeHls &&
		!youtubeHlsOptions.includes(env.enableDeprecatedYoutubeHls)
	) {
...
const reloadEnvs = async contents => {
	...
	const parsed = await validateEnvs(loadEnvs(candidate));
	...
	reloadEnvs(contents)
		...
		.catch(e => {
			console.error(
				`${Yellow(
					"[!]"
				)} Failed reloading environment variables at ${new Date().toISOString()}.`
			);
...
export const setupEnvWatcher = () => {
	if (cluster.isPrimary) {
		...
		const isFile = !isURL(envFile) || new URL(envFile).protocol === "file:";
