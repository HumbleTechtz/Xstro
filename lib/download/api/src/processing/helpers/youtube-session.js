...
const validateSession = sessionResponse => {
	...
	if (sessionResponse.potoken.length < 160) {
		console.error(
			`${Yellow(
				"[!]"
			)} poToken is too short and might not work (${new Date().toISOString()})`
		);
	}
};
...
const loadSession = async () => {
	...
	const newSession = await fetch(sessionServerUrl, {
	...
	loadSession()
		...
		.catch(e => {
			console.error(
				`${Yellow(
					"[!]"
				)} Failed loading poToken & visitor_data at ${new Date().toISOString()}.`
			);
