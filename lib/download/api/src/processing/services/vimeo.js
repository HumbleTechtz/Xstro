...
const genericHeaders = {
	...
	"User-Agent":
		"Vimeo/11.13.0 (com.vimeo; build:250619.102023.0; iOS 18.5.0) Alamofire/5.9.0 VimeoNetworking/5.0.0",
	Authorization:
...
const getDirectLink = async (data, quality, subtitleLang) => {
	...
	if (subtitleLang && data.config_url) {
		...
		if (config && config.request?.text_tracks?.length) {
			subtitles = config.request.text_tracks.find(t =>
...
const getHLS = async (configURL, obj) => {
	...
	const urlMasterHLS =
		api.request?.files?.hls?.cdns?.akfire_interconnect_quic?.url;
	...
	if (obj.quality < resolutionMatch[variants[0]?.resolution?.width]) {
		bestQuality = variants.find(
			v => obj.quality === resolutionMatch[v.resolution.width]
		);
	}
	...
	if (audioPath) {
		urls = [urls, expandLink(audioPath)];
