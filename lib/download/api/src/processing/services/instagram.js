...
export default function instagram(obj) {
	...
	async function getGQLParams(id, cookie) {
		...
		return {
			headers: {
				...
				"X-FB-LSD": lsd,
				"X-CSRFToken": csrf,
				"X-Bloks-Version-Id": getObjectFromEntries("WebBloksVersioningID", html)
					?.versioningID,
				"x-asbd-id": 129477,
				cookie: anon_cookie,
			},
			body: {
				...
				lsd,
				jazoest:
					getNumberFromQuery("jazoest", html) || Math.floor(Math.random() * 10000),
	...
	async function getErrorContext(id) {
		try {
			...
			const [, mediaId, mediaOwnerId] =
				response.match(/"media_id":\s*?"(\d+)","media_owner_id":\s*?"(\d+)"/) || [];

			if (mediaId && mediaOwnerId) {
				const rulingURL = new URL(
		...
		if (carousel) {
			const picker = carousel
				...
				.map((e, i) => {
					...
					if (type === "video") {
						const video = e.video_versions.reduce((a, b) =>
			...
			const video = data.video_versions.reduce((a, b) =>
				a.width * a.height < b.width * b.height ? b : a
			);
			return {
				urls: video.url,
	...
	async function getPost(id, alwaysProxy) {
		...
		try {
			...
			if (media_id && cookie && !hasData(data))
				data = await requestMobileApi(media_id, { cookie });
	...
	async function getStory(username, id) {
		...
		try {
			...
			media = data?.data?.xdt_api__v1__feed__reels_media?.reels_media?.find(
				m => m.id === userId
			);
		} catch {}
