...
const extractVideo = async ({ media, filename, dispatcher }) => {
	...
	const video = HLS.parse(masterHLS)?.variants?.reduce((a, b) =>
...
export default async function ({ user, post, alwaysProxy, dispatcher }) {
	const apiEndpoint = new URL(
		"https://public.api.bsky.app/xrpc/app.bsky.feed.getPostThread?depth=0&parentHeight=0"
	);
	apiEndpoint.searchParams.set("uri", `at://${user}/app.bsky.feed.post/${post}`);
	...
	switch (embedType) {
		...
		case "app.bsky.embed.recordWithMedia#view":
			if (
				getPost?.thread?.post?.embed?.media?.$type ===
