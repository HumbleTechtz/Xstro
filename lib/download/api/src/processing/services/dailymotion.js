...
const getToken = async () => {
	...
	const req = await fetch("https://graphql.api.dailymotion.com/oauth/token", {
		...
		headers: {
			...
			Authorization:
				"Basic MGQyZDgyNjQwOWFmOWU3MmRiNWQ6ODcxNmJmYTVjYmEwMmUwMGJkYTVmYTg1NTliNDIwMzQ3NzIyYWMzYQ==",
		},
...
export default async function ({ id }) {
	...
	const manifest = await fetch(media.hlsURL)
		.then(r => r.text())
		.catch(() => {});
	if (!manifest) return { error: "fetch.fail" };

	const bestQuality = HLSParser.parse(manifest)
		.variants.filter(v => v.codecs.includes("avc1"))
