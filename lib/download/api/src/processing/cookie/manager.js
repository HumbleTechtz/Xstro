	...
	try {
		...
		for (const serviceName in cookies) {
			if (!VALID_SERVICES.has(serviceName)) {
				console.warn(
					`${Yellow("[!]")} ignoring unknown service in cookie file: ${serviceName}`
				);
			} else if (!Array.isArray(cookies[serviceName])) {
				console.warn(
					`${Yellow(
						"[!]"
					)} ${serviceName} in cookies file is not an array, ignoring it`
				);
			} else if (cookies[serviceName].some(c => typeof c !== "string")) {
				console.warn(
					`${Yellow(
						"[!]"
					)} some cookie for ${serviceName} contains non-string value in cookies file`
				);
			} else continue;
