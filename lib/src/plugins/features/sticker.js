...
import { removeStickerCmd, setStickerCmd } from "../../models/sticker.js";
Command({
    name: "setcmd",
    fromMe: true,
    isGroup: false,
    ...
    function: async (message, match) => {
        ...
        const cmds = commands.map(cmd => cmd.name?.toString().split(/[\p{S}\p{P}]/gu)[5]);
...
Command({
    ...
    function: async (message, match) => {
        if (!match)
            return message.send("_Provide a command name, eg ping_");
        const cmdname = match?.trim().toLowerCase();
        const cmds = commands.map(cmd => cmd.name?.toString().split(/[\p{S}\p{P}]/gu)[5]);
        if (!cmds.includes(cmdname))
            return await message.send("_This command does not exist_");
