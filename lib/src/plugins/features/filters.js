...
import { Filters, setFilter, delFilter, getAllFilters, } from "../../models/filter.js";
import { escapeRegex } from "../group/antiword.js";
Command({
    name: "filter",
    fromMe: true,
    isGroup: false,
    ...
    type: "filter",
    function: async (msg) => {
        return await msg.send(`\`\`\`Filters Menu:
...
Command({
    ...
    function: async (msg, match) => {
        ...
        if (!deleted)
            return await msg.send("Filter not found");
...
Command({
    ...
    function: async (msg) => {
        ...
        if (keyword.isGroup) {
            if (!msg.isGroup)
                return;
            if (!msg.mention?.includes(msg.owner.jid) ||
