...
import { setSettings } from "../../models/settings.js";
Command({
    name: "setprefix",
    fromMe: true,
    isGroup: false,
    ...
    function: async (msgs, args) => {
        if (!args) {
            return await msgs.send(`_Prefix is needed, eg ${msgs.prefix[0]}setprefix ,_`);
        }
        await setSettings("prefix", [args]);
        return await msgs.send(`_Bot Prefix updated to "${args}"_\nUsage: ${args}ping`);
    },
});
Command({
    name: "mode",
