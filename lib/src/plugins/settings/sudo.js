...
import { delsudo, getSudo, setSudo } from "../../models/settings.js";
Command({
    name: "setsudo",
    fromMe: true,
    isGroup: false,
    ...
    function: async (msg, args) => {
        if (!args)
            return msg.send("_Provide number to sudo_");
        if (msg.isGroup)
            return msg.send(`_${msg.sender.split("@")[0]} You cannot sudo inside a Group, Please this command should be used in Personal Chats_`);
        const sudoList = await getSudo();
        const userToSudo = await msg.parseId(args);
        if (!userToSudo)
            return msg.send(`_User is invaild_`);
        if (sudoList.includes(userToSudo))
            return msg.send("_Already a sudo user_");
...
Command({
    ...
    function: async (msg) => {
        const settings = await getSudo();
        if (!settings.length)
            return msg.send("_No sudo users found._");
