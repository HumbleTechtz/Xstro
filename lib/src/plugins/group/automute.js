...
import { setAutoMute, delAutoMute, getAutoMute, } from "../../models/automute.js";
import { isValidTimeString } from "../../utils/constants.js";
Command({
    name: "amute",
    fromMe: false,
    isGroup: true,
    ...
    function: async (msg, args) => {
        ...
        if (!isValidTimeString(startTime)) {
            return await msg.send("Invalid time format. Use format like `5:30pm` or `7:00am`.");
        }
        await setAutoMute(jid, startTime);
        return await msg.send(`_Group will be auto-muted everyday at ${startTime.toUpperCase()}._`);
    },
});
Command({
    name: "aunmute",
    ...
    function: async (msg, args) => {
        ...
        if (!isValidTimeString(endTime)) {
            return await msg.send("Invalid time format. Use format like `6:30am` or `8:00pm`.");
        }
        const existing = await getAutoMute(jid);
        const startTime = existing?.startTime ?? null;
        await setAutoMute(jid, startTime, endTime);
        return await msg.send(`_Group will be auto-unmuted everyday at ${endTime.toUpperCase()}._`);
    },
});
Command({
    name: "delmute",
...
Command({
    ...
    function: async (msg) => {
        ...
        return await msg.send(`_Automute is currently set${startTime ? ` from ${startTime}` : ""}${endTime ? ` to ${endTime}` : ""}._`);
    },
});
