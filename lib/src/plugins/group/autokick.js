...
import { addAutoKick, delAutoKick, getAutoKick, } from "../../models/autokick.js";
Command({
    name: "autokick",
    fromMe: true,
    isGroup: true,
    ...
    function: async (msg, args) => {
        ...
        if (!parsedUser)
            return msg.send("_Invalid user provided_");
        const { jid, lid, exists } = await msg
            .onWhatsApp(parsedUser)
            .then(v => v[0]);
        if (!exists)
            return msg.send("_User does not exist on WhatsApp_");
        if (subcmd === "add") {
            await addAutoKick(groupJid, jid, lid, [jid]);
