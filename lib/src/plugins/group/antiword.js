...
import { setAntiWord, getAntiword } from "../../models/antiword.js";
...
${prefix}antiword set word1, word2 — Set blocked words`);
        ...
        if (lcCmd === "on" || lcCmd === "off") {
            ...
            return message.send(`_Antiword filter has been ${lcCmd === "on" ? "enabled" : "disabled"}._`);
        }
        if (lcCmd === "get") {
            const { words = [] } = (await getAntiword(jid)) || {};
            return message.send(words.length
        ...
        if (lcCmd === "set") {
            const words = rest
                ...
                .filter(Boolean);
            if (!words.length)
                return message.send("_No valid words detected._");
...
Command({
    ...
    function: async (msg) => {
        ...
        if (!record?.status || !record.words?.length)
            return;
