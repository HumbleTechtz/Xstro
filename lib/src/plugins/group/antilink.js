...
import { delAntilink, getAntilink, setAntilink, } from "../../models/antilink.js";
import { isAdmin, isBotAdmin } from "../../utils/constants.js";
Command({
    name: "antilink",
    fromMe: false,
    isGroup: true,
...
Command({
    ...
    function: async (msg) => {
        if (!msg.isGroup || !msg?.text)
            return;
        if (msg.key.fromMe || msg.sudo)
            return;
        if (!(await isBotAdmin(msg)) || (await isAdmin(msg.jid, msg.sender)))
            return;
        ...
        if (antilink.links?.length) {
            hasProhibitedLink = antilink.links.some((link) => text.includes(link.toLowerCase()));
        }
        else {
            hasProhibitedLink =
        ...
        if (antilink.mode === true) {
            ...
            await msg.send(`_@${msg.sender.split("@")[0]} was removed for sending a prohibited link_`, { mentions: [msg.sender] });
        }
        else {
