...
import { adminCheck } from "../../utils/constants.js";
Command({
    name: "add",
    fromMe: true,
    isGroup: true,
    ...
    function: async (message, match) => {
        if (!(await adminCheck(message)))
            return;
        if (!match)
            return message.send("_Provide a valid number_");
...
Command({
    ...
    function: async (message, match) => {
        if (!(await adminCheck(message)))
            return;
        if (!match)
            return message.send("_Provide a valid number or mention_");
        const user = await message.parseId(match);
        if (!user)
            return message.send("_Invalid number or mention_");
...
Command({
    ...
    function: async (message, match) => {
        if (!match)
            return message.send("_Provide group name_");
...
Command({
    ...
    function: async (message, match) => {
        ...
        return await message.relayMessage(message.jid, {
            extendedTextMessage: {
...
Command({
    ...
    function: async (message, match) => {
        if (!(await adminCheck(message)))
            return;
        if (!match)
            return message.send("_Provide new group name_");
...
Command({
    ...
    function: async (message, match) => {
        if (!(await adminCheck(message)))
            return;
        if (!match)
            return message.send("_Provide new group description_");
...
Command({
    ...
    function: async (message) => {
        ...
        if (metadata.announce)
            return message.send("_Group already muted_");
...
Command({
    ...
    function: async (message) => {
        ...
        const metadata = await cachedGroupMetadata(message.jid);
        if (!metadata.announce)
            return message.send("_Group already unmuted_");
...
Command({
    ...
    function: async (message, match) => {
        ...
        const options = optionsRaw
            ...
            .filter(Boolean);
        if (options.length < 2)
            return message.send("_Add at least 2 options_");
...
Command({
    ...
    function: async (message) => {
        ...
        const participants = requests.map(p => p.id);
        await message.groupRequestParticipantsUpdate(message.jid, participants, "approve");
...
Command({
    ...
    function: async (message) => {
        ...
        const participants = requests.map(p => p.id);
        await message.groupRequestParticipantsUpdate(message.jid, participants, "reject");
