...
import { convertToMp3, toPTT, toVideo, audioToBlackVideo, flipMedia, cropToCircle, createSticker, trimVideo, } from "../../utils/ffmpeg.mjs";
Command({
    name: "ptt",
    fromMe: false,
    isGroup: false,
...
Command({
    ...
    function: async (message, args) => {
        ...
        if (msg?.type !== "videoMessage" &&
            ...
            return message.send("_Reply a video, image or sticker_\n_" +
                message.prefix[0] +
        ...
        const sticker = await createSticker(await message.downloadM(msg), author?.trim() || "Astro", packname?.trim() || "Xstro");
        return await message.sendMessage(message.jid, {
            sticker,
            mimetype: "image/webp",
        });
...
Command({
    ...
    function: async (message, match) => {
        ...
        if (!startTime || !endTime)
            return message.send(`Usage:\n${message.prefix}trim 00:00:05|00:01:08`);
        const trimmedVideo = await trimVideo(await message.downloadM(msg), startTime, endTime);
