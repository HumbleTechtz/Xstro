...
import { voxnews, wabetanews, technews } from "../utils/scraper.mjs";
Command({
    name: "news",
    fromMe: false,
    isGroup: false,
    ...
    function: async (message) => {
        ...
        if (!news)
            return message.send("_No news avaliable_");
...
Command({
    ...
    function: async (message) => {
        ...
        if (!wabetaInfo)
            return message.send("_No WA updates avaliable_");
...
Command({
    ...
    function: async (message) => {
        ...
        if (!techInfo)
            return message.send("_No tech news updates!_");
