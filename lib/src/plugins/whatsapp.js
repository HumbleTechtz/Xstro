...
Command({
    ...
    function: async (message, match) => {
        if (!match)
            return message.send("No bio text provided");
...
Command({
    ...
    function: async (message, match) => {
        if (!match)
            return message.send("No name provided");
...
Command({
    ...
    function: async (message, match) => {
        ...
        if (!jid)
            return message.send("```No user specified to block```");
...
Command({
    ...
    function: async (message, match) => {
        const jid = await message.parseId(match);
        if (!jid)
            return message.send("```No user specified to unblock```");
...
Command({
    ...
    function: async (message) => {
        const msg = message.quoted;
        if (!msg || !msg.viewOnce)
            return message.send("```Reply a Viewonce```");
        if (msg.message) {
...
Command({
    ...
    function: async (message, match) => {
        ...
        if (!match)
            return message.send(`Usage: $${message.prefix[0]}edit Hello.`);
...
Command({
    ...
    function: async (message) => {
        ...
        if (!users)
            return message.send("No blocked users found");
...
Command({
    ...
    function: async (message) => {
        const msg = message.quoted;
        if (!msg?.broadcast)
            return message.send("No status replied to");
...
Command({
    ...
    function: async (message, match) => {
        const msg = message.quoted;
        if (!msg)
            return message.send("No message quoted to forward");
        const jid = await message.parseId(match);
        if (!jid)
            return message.send("No user specified to forward");
