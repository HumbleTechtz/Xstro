import { Command, commands } from "../messaging/plugin.js";
import { platform, totalmem, freemem } from "node:os";
import { fancy, formatBytes, formatRuntime } from "../utils/constants.js";
...
Command({
    ...
    function: async (message) => {
        const cmds = commands.filter(cmd => cmd.name &&
            !cmd.dontAddCommandList &&
            !cmd.name.toString().includes("undefined")).length;
...
╰─────────────\`\`\`\n`.trim();
        ...
        sortedTypes.forEach(type => {
            ...
            sortedCommands.forEach((cmd) => {
                menuInfo += `│${fancy(totalCommands)}· ${fancy(cmd)}\n`;
                totalCommands++;
            });
            menuInfo += `╰────────────\n`;
        });
