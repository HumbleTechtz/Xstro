import { fetch, urlBuffer } from "../utils/fetch.mjs";
import { Command } from "../messaging/plugin.js";
import { convertToMp3 } from "../utils/ffmpeg.mjs";
...
Command({
    ...
    function: async (msg, args) => {
        if (!args)
            return await msg.send(`_Provide an App name_`);
        await msg.react("⬇️");
        const data = await fetch(`https://bk9.fun/download/apk?id=${args}`).then(res => JSON.parse(res).BK9);
        if (data.icon) {
...
Command({
    ...
    function: async (msg, args) => {
        ...
        if (!url || !args)
            return await msg.send("_Provide a valid facebook link_");
        await msg.react("⬇️");
        const data = await fetch(`https://bk9.fun/download/fb?url=${new URL(args)}`).then(res => JSON.parse(res).BK9);
        const { sd, hd } = data;
        if (hd) {
...
Command({
    ...
    function: async (msg, args) => {
        ...
        const data = await fetch(`https://bk9.fun/download/instagram?url=${new URL(args)}`).then(res => JSON.parse(res).BK9);
...
Command({
    ...
    function: async (msg, args) => {
        if (!args)
            return await msg.send("_Provide a valid twitter link_");
        ...
        if (!url)
            return await msg.send("_Provide a valid twitter link_");
        await msg.react("⬇️");
        const data = await fetch(`https://bk9.fun/download/twitter?url=${new URL(args)}`).then(res => JSON.parse(res).BK9);
...
Command({
    ...
    function: async (msg, args) => {
        if (!args) {
            msg.send("_Provide any MP3 URL. Note: This works only for direct audio file links, not social media or streaming sites._");
            return;
        }
        await msg.react("⬇️");
        try {
...
Command({
    ...
    function: async (msg, args) => {
        if (!args) {
            msg.send("_Provide any mp4 URL. Note: This works only for direct video file links, not social media or YouTube._");
            return;
        }
        await msg.react("⬇️");
        try {
...
Command({
    ...
    function: async (msg, args) => {
        ...
        if (!url || !args)
            return await msg.send("_Provide a valid tiktok link_");
        await msg.react("⬇️");
        const data = await fetch(`https://bk9.fun/download/tiktok?url=${new URL(args)}`).then(res => JSON.parse(res).BK9);
...
Command({
    ...
    type: "download",
    function: async (msg, args) => {
        const url = /^https?:\/\/(www\.|m\.)?(youtube\.com|youtu\.be)(\/|$)/.test(args ?? "");
        if (!url || !args)
            return await msg.send("_Provide a valid youtube link_");
        await msg.react("⬇️");
        const data = await fetch(`https://bk9.fun/download/ytmp3?url=${new URL(args)}&type=mp3`).then(res => JSON.parse(res).BK9);
        const { image, title, downloadUrl } = data;
        const audio = await convertToMp3(await urlBuffer(downloadUrl));
        await msg.sendMessage(msg.jid, {
            audio: audio,
            ptt: false,
            mimetype: "audio/mpeg",
