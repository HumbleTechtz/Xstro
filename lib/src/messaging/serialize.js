import { downloadMediaMessage, getDevice, isJidGroup, isJidUser, isLidUser, jidNormalizedUser, normalizeMessageContent, } from "baileys";
import { getSettings } from "../models/index.js";
import { forwardM, getMessageContent, getQuotedContent, isBotAdmin, isMediaMessage, prepareMessage, } from "../utils/index.js";
export async function serialize(client, WAMessage) {
    const normalizedMessages = {
        ...WAMessage,
        message: normalizeMessageContent(WAMessage?.message),
    };
    const { key, message, broadcast, ...messages } = normalizedMessages;
    const { prefix, mode, banned, disablecmd, disablegc, disabledm, sudo } = await getSettings();
    const jid = key.remoteJid;
    const isGroup = isJidGroup(jid);
    const jidOwner = jidNormalizedUser(client?.user?.id);
    const lidOwner = jidNormalizedUser(client?.user?.lid);
    const sender = isJidGroup(jid) || broadcast
        ? key.participant
        : key.fromMe
            ? jidOwner
            : jid;
    ...
    const isSudoUser = (user) => sudo.includes(user ?? "") || user === jidOwner || user === lidOwner;
    ...
    return {
        ...
        downloadM: async function (message) {
            return await downloadMediaMessage(message ?? this, "buffer", {});
        },
        forward: async function (jid, message, options) {
        ...
        editM: async function (text, msg) {
            if (isMediaMessage(this)) {
                const media = await this.downloadM(msg ?? this);
                return await client.sendMessage(this.jid, this.mtype === "imageMessage"
        ...
        deleteM: async function (key) {
            const canDeleteForAll = this.key.fromMe || (this.isGroup && (await isBotAdmin(this)));
        ...
        parseId: async function (id) {
            if (id) {
                if (isLidUser(id) || isJidUser(id))
                    return id;
