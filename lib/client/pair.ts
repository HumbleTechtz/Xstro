...
export default async function (sock: WASocket) {
	if (!sock.authState?.creds?.registered) {
		...
		if (phoneNumber.length < 11) {
			console.error(
				"Invalid phone number. Please provide a valid phone number in the config."
			);
			shutdown();
		}
		await new Promise(resolve => setTimeout(resolve, 2000));
		console.log(
			`Pairing Code: ${await sock.requestPairingCode(phoneNumber, "ASTROX11")}`
		);
