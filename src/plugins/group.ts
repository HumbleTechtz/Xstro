...
// 	if (!user?.endsWith('@lid') && !user?.endsWith('@s.whatsapp.net')) {
// 		if (await client?.onWhatsApp(`${match}'@s.whatsapp.net'`))
...
// 		const user = await getUser(message, match);
// 		if (!user) return;
